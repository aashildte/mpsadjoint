...
def eval_cb_checkpoint(cost_cur, control_params, tracked_quantities, mesh):
    ...
    *active_strain, theta = control_params
    ...
    for t, active in enumerate(active_strain):
        ...
        print(f"****** Average active strain at time step {t}: {active_str_t:.6f}", flush=True)
...
def load_data(output_folder, U, TH, num_time_steps):
    ...
    states = read_states_from_file(TH, output_folder + "/displacement.xdmf", output_folder + "/pressure.xdmf", num_time_steps)
...
def solve_iteratively(
    ...
    while heap:
        ...
        if output_folder is not None and data_exist(output_folder_t):
            print(
                f"Loading inversion results for time step {time_step}; iteration {iteration_num} / {len(u_data)}", flush=True
            )
            active_strain, theta, states = load_data(output_folder_t, U, TH, 1)
        else:
            print(
                f"Starting inversion for time step {time_step}; iteration {iteration_num} / {len(u_data)}", flush=True
            )
            (
                active_strain,
                theta,
                ...
                geometry, [u_data[time_step]], active_strain, theta, states, number_of_iterations,
            )
    ...
    if output_folder is not None and not data_exist(output_folder):
        write_inversion_results(
            geometry.mesh, active_strain_over_time, theta, states_over_time, output_folder
        )
    ...
    for t, (active, theta_t, state) in enumerate(zip(
        active_strain_over_time, theta_over_time, states_over_time
    )):
        print("Solving for time step ", t, flush=True)      
        newtonsolver, forward_problems, state2 = define_forward_problems(geometry, [active], theta_t, init_states=[state])
...
def solve_inverse_problem(
    ...
    if output_folder is None:
        ...
        data_path = output_folder + f"/combined_{number_of_iterations_iterative}_{number_of_iterations_combined}"
    ...
    active_strain, theta, states, tracked_quantities = \
    solve_pdeconstrained_optimization_problem(
    ...
    if output_folder is not None:
        write_inversion_results(
            ...
            theta,
            states,
            output_folder + f"/combined_{number_of_iterations_iterative}_{number_of_iterations_combined}",
        )
...
def solve_inverse_problem_phase_3(
    ...
    for folder, u_d in zip(folders, u_data):
        ...
        data_path = folder + f"/combined_{number_of_iterations_iterative}_{number_of_iterations_combined}"
    ...
    for i, theta_t in enumerate(theta_all):
        print("Dose: ", i)
        for t, (active, state) in enumerate(zip(
            active_all[i*T:(i+1)*T], states_all[i*T:(i+1)*T]
        )):
            print("Solving for time step ", t, flush=True)      
            newtonsolver, forward_problems, state2 = define_forward_problems(geometry, [active], theta_t, init_states=[state])
    ...
    active_all, theta, states, tracked_quantities = \
    solve_pdeconstrained_optimization_problem(
        geometry, u_data_all, active_all, theta_avg, states_all_syncronized, number_of_iterations,
    )
    ...
    for i, output_folder in enumerate(folders):
        ...
        write_inversion_results(
            ...
            theta,
            states_per_dose,
            output_folder + f"/combined_phase3_{number_of_iterations_iterative}_{number_of_iterations_combined}_{number_of_iterations}",
        )
